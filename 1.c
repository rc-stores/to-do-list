...
void show_Task()
{
	char line[128], strday[2], strmon[2], *timestr;
	int day, month, year;
	time_t rawtime;
	struct tm *curr_t;
	int mode=0;
	...
	if(scanf("%d.%d.%d", &day, &month, &year)==3) 
		timestr="00:00";	
	else if(scanf("%d.%d.%d %s", &day, &month, &year, timestr)==4) 
		mode=1;	
...
void print_Task(char *line, int mode) 
{
	...
	if((fp=fopen("todolist.txt", "r"))==NULL) {
	    ...
	    if(n==0)
	        printf("There is no plans set on this date.\n");	
}

char *format(char *line) 
{
	char result[1256];
	char time[16], task[128];
	
	sscanf(line, "%*s %s %s", time, task);
	char *p=line+17;
	for(int i=0; p[i]!='\0'; i++) {
		task[i]=p[i];
	}
	printf("%s %s\n", time, task);
	sprintf(result, "This day at %s: %s\n", time, task);
